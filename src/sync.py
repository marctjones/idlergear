...
class ProjectSync:
    ...
    def sync_push(self, include_untracked: bool = False) -> dict:
        ...
        returncode, stdout, stderr = self._run_git("rev-parse", "--verify", sync_branch, check=False)
    ...
    def sync_pull(self, cleanup: bool = True) -> dict:
        ...
        returncode, stdout, stderr = self._run_git(
            "ls-remote", "--heads", "origin", sync_branch,
            check=False
        )
        ...
        if cleanup:
            ...
            returncode, _, _ = self._run_git("rev-parse", "--verify", sync_branch, check=False)
            if returncode == 0:
